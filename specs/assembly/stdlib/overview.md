...
For more information about the `__vectorcall` calling convention, see [__vectorcall](../cpp/vectorcall?view=msvc-170).
...
Integer arguments are passed in registers RCX, RDX, R8, and R9. Floating point arguments are passed in XMM0L, XMM1L, XMM2L, and XMM3L. 16-byte arguments are passed by reference. Parameter passing is described in detail in [Parameter passing](#parameter-passing). These registers, and RAX, R10, R11, XMM4, and XMM5, are considered volatile, or potentially changed by a callee on return. Register usage is documented in detail in [x64 register usage](x64-software-conventions?view=msvc-170#x64-register-usage) and [Caller/callee saved registers](#callercallee-saved-registers).
...
Most structures are aligned to their natural alignment. The primary exceptions are the stack pointer and `malloc` or `alloca` memory, which are 16-byte aligned to aid performance. Alignment above 16 bytes must be done manually. Since 16 bytes is a common alignment size for XMM operations, this value should work for most code. For more information about structure layout and alignment, see [x64 type and storage layout](x64-software-conventions?view=msvc-170#x64-type-and-storage-layout). For information about the stack layout, see [x64 stack usage](stack-usage?view=msvc-170).
...
[__m128](../cpp/m128?view=msvc-170) types, arrays, and strings are never passed by immediate value. Instead, a pointer is passed to memory allocated by the caller. Structs and unions of size 8, 16, 32, or 64 bits, and `__m64` types, are passed as if they were integers of the same size. Structs or unions of other sizes are passed as a pointer to memory allocated by the caller. For these aggregate types passed as a pointer, including `__m128`, the caller-allocated temporary memory must be 16-byte aligned.
...
Parameter typefifth and higherfourththirdsecondleftmostfloating-pointstackXMM3XMM2XMM1XMM0integerstackR9R8RDXRCXAggregates (8, 16, 32, or 64 bits) and `__m64`stackR9R8RDXRCXOther aggregates, as pointersstackR9R8RDXRCX`__m128`, as a pointerstackR9R8RDXRCX
...
A scalar return value that can fit into 64 bits, including the `__m64` type, is returned through RAX. Nonscalar types including floats, doubles, and vector types such as [__m128](../cpp/m128?view=msvc-170), [__m128i](../cpp/m128i?view=msvc-170), [__m128d](../cpp/m128d?view=msvc-170) are returned in XMM0. The state of unused bits in the value returned in RAX or XMM0 is undefined.
...
Register[bits]SettingFPCSR[0:6]Exception masks all 1s (all exceptions masked)FPCSR[7]Reserved - 0FPCSR[8:9]Precision Control - 10B (double precision)FPCSR[10:11]Rounding control - 0 (round to nearest)FPCSR[12]Infinity control - 0 (not used)
...
Register[bits]SettingMXCSR[6]Denormals are zeros - 0MXCSR[7:12]Exception masks all 1s (all exceptions masked)MXCSR[13:14]Rounding control - 0 (round to nearest)MXCSR[15]Flush to zero for masked underflow - 0 (off)
...
When you include `setjmpex.h` or `setjmp.h`, all calls to [setjmp](../c-runtime-library/reference/setjmp?view=msvc-170) or [longjmp](../c-runtime-library/reference/longjmp?view=msvc-170) result in an unwind that invokes destructors and `__finally` calls. This behavior differs from x86, where including `setjmp.h` results in `__finally` clauses and destructors not being invoked.
...
- [Overview of x64 ABI conventions](x64-software-conventions?view=msvc-170)
