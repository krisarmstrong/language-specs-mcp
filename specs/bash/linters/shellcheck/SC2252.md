ShellCheck: SC2252 – You probably wanted `&&` here, otherwise it's always true.

# SC2252 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2252)
...
This statement is identical to `! [ "$1" = foo ] || [ "$1" = bar ]`, which also works correctly (by [De Morgan's
law](https://en.wikipedia.org/wiki/De_Morgan%27s_laws))

This warning is equivalent to [SC2055](SC2055) and [SC2056](SC2056), which trigger for intra-`test` expressions and arithmetic contexts respectively.
