ShellCheck: SC2116 – Useless echo? Instead of `cmd $(echo foo)`, just use `cmd foo`.

# SC2116 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2116)
...
```
#cb1-1greeting=$(echo "Hello, $name")
#cb1-2# or
#cb1-3tar czf "$(echo "$(date +%F).tar.gz")" *
```

### Correct code:

```
#cb2-1greeting="Hello, $name"
#cb2-2# or
#cb2-3tar czf "$(date +%F).tar.gz" *
```
...
```
#cb4-1unexpanded='var\tvalue'
#cb4-2expanded="$(echo "$var")"
```
