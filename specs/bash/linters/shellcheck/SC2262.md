ShellCheck: SC2262 – This alias can't be defined and used in the same parsing unit. Use a function instead.

# SC2262 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2262)
...
```
#cb1-1function checksum() {
#cb1-2  type md5 && alias md5sum=md5
#cb1-3  md5sum "$@"  # This calls `md5sum`, not `md5`
#cb1-4}
```
...
```
#cb2-1function checksum() {
#cb2-2  type md5 && md5sum() { md5 "$@"; }
#cb2-3  md5sum "$@"  # Now this would call `md5` when applicable
#cb2-4}
```
...
You can [ignore](ignore) this warning with a directive. All warnings may always be disabled either before the relevant command or before any outer compound commands, but in this case it's especially useful:
...
- Unix&Linux SE: [bash
aliases do not expand even with shopt expand_aliases](https://unix.stackexchange.com/questions/502257/bash-aliases-do-not-expand-even-with-shopt-expand-aliases)
