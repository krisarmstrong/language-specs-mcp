ShellCheck: SC2012 – Use `find` instead of `ls` to better handle non-alphanumeric filenames.

# SC2012 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2012)
...
```
#cb1-1ls -l | grep " $USER " | grep '\.txt$'
```

```
#cb2-1NUMGZ="$(ls -l *.gz | wc -l)"
```

### Correct code:

```
#cb3-1find ./*.txt -user "$USER"  # Using the names of the files
```

```
#cb4-1gz_files=(*.gz)
#cb4-2numgz=${#gz_files[@]} # Sometimes, you just need a count
```
...
If the information is intended for the user and not for processing (`ls -l ~/dir | nl; echo "Ok to delete these files?"`) you can ignore this error with a [directive](directive).
