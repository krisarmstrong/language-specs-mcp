ShellCheck: SC2069 – To redirect stdout+stderr, `2>&1` must be last (or use `{ cmd > file; } 2>&1` to clarify).

# SC2069 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2069)
...
```
#cb1-1firefox 2>&1 > /dev/null
```

### Correct code:

```
#cb2-1firefox > /dev/null 2>&1
```
...
- [Bash
Pitfalls: somecmd 2>&1 >>logfile](https://mywiki.wooledge.org/BashPitfalls#pf43)
- [BashFaq: Tell me
all about 2>&1 -- what's the difference between 2>&1
>foo and >foo 2>&1, and when do I use which?](https://mywiki.wooledge.org/BashFAQ/055)
