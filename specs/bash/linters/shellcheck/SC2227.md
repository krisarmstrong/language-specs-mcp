ShellCheck: SC2227 – Redirection applies to the find command itself. Rewrite to work per action (or move to end).

# SC2227 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2227)
...
```
#cb1-1find . -name '*.ppm' -exec pnmtopng {} > {}.png \;
```

### Correct code:

```
#cb2-1find . -name '*.ppm' -exec sh -c 'pnmtopng "$1" > "$1.png"' _ {} \;
```
