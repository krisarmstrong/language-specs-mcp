ShellCheck: SC2094 – Make sure not to read and write the same file in the same pipeline.

# SC2094 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2094)
...
```
#cb1-1grep foo file.txt | sed -e 's/foo/bar/g' > file.txt
```

### Correct code:

```
#cb2-1grep foo file.txt  | sed -e 's/foo/bar/g' > tmpfile && mv tmpfile file.txt
```
...
- [StackOverflow:
bash redirect input from file back into same file](https://stackoverflow.com/questions/6696842/bash-redirect-input-from-file-back-into-same-file)
- [BashPitfalls:
cat file | sed s/foo/bar/ > file](https://mywiki.wooledge.org/BashPitfalls#pf13)
