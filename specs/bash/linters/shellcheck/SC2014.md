ShellCheck: SC2014 – This will expand once before find runs, not per file found.

# SC2014 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2014)
...
```
#cb1-1find . -name '*.tar' -exec tar xf {} -C "$(dirname {})" \;
```

### Correct code:

```
#cb2-1find . -name '*.tar' -exec sh -c 'tar xf "$1" -C "$(dirname "$1")"' _ {} \;
```
...
```
#cb3-1$ sh -c 'echo "$1 is in $(dirname "$1")"' _ "mydir/myfile"
#cb3-2mydir/myfile is in mydir
```
