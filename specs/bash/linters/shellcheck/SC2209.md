ShellCheck: SC2209 – Use `var=$(command)` to assign output (or quote to assign string).

# SC2209 – ShellCheck Wiki

[See this page on GitHub](https://github.com/koalaman/shellcheck/wiki/SC2209)
...
```
#cb1-1user=whoami         # Want to run whoami and assign output
#cb1-2
#cb1-3PAGER=cat git log   # Want to assign the string "cat"
```
...
This warning triggers generally when a variable is assigned an unquoted command name (from a list of hard coded names). See related warning [SC2037](SC2037) which detects the same kind of error through the patterns `var=value -flag` and `var=value *glob*`.

### Related resources:

- [BashFaq: How can I
store the return value and/or output of a command in a
variable?](https://mywiki.wooledge.org/BashFAQ/002)
- [StackOverflow:
