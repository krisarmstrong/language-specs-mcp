...
- [Report feedback](https://survey.alchemer.com/s3/7634825/MDN-baseline-feedback?page=%2Fen-US%2Fdocs%2FWeb%2FAPI%2FHTML_Sanitizer_API&level=not)
...
Injecting untrusted HTML can make a site vulnerable to various [types of attacks](/en-US/docs/Web/Security/Attacks). In particular, [cross-site scripting (XSS) attacks](/en-US/docs/Web/Security/Attacks/XSS) work by injecting untrusted HTML into the DOM that then executes JavaScript in the context of the current origin — allowing malicious code to run as though it was served from the site's origin. These attacks can be mitigated by removing unsafe HTML elements and attributes before they are injected into the DOM.
...
### [Sanitization methods](#sanitization_methods)

The HTML Sanitizer API provides XSS-safe and XSS-unsafe methods for injecting HTML strings into an [Element](/en-US/docs/Web/API/Element) or a [ShadowRoot](/en-US/docs/Web/API/ShadowRoot), and for parsing HTML into a [Document](/en-US/docs/Web/API/Document).

- Safe methods: [Element.setHTML()](/en-US/docs/Web/API/Element/setHTML), [ShadowRoot.setHTML()](/en-US/docs/Web/API/ShadowRoot/setHTML), and [Document.parseHTML()](/en-US/docs/Web/API/Document/parseHTML_static).
- Unsafe methods: [Element.setHTMLUnsafe()](/en-US/docs/Web/API/Element/setHTMLUnsafe), [ShadowRoot.setHTMLUnsafe()](/en-US/docs/Web/API/ShadowRoot/setHTMLUnsafe), and [Document.parseHTMLUnsafe()](/en-US/docs/Web/API/Document/parseHTMLUnsafe_static).

All the methods take the HTML to be injected and an optional [sanitizer configuration](#sanitizer_configuration) as arguments. The configuration defines the HTML entities that will be filtered out of the input before it is injected. The [Element](/en-US/docs/Web/API/Element) methods are context aware, and will additionally drop any elements that the HTML specification does not allow in the target element.

The safe methods always remove XSS-unsafe elements and attributes. If no sanitizer is passed as a parameter they will use the default sanitizer configuration, which allows all elements and attributes except those that are known to be unsafe, such as [<script>](/en-US/docs/Web/HTML/Reference/Elements/script) elements and `onclick` event handlers. If a custom sanitizer is used, it is implicitly updated to remove any elements and attributes that are not XSS-safe (note that the passed sanitizer is not modified, and might still allow unsafe entities if used with an unsafe method).

The safe methods should be used instead of [Element.innerHTML](/en-US/docs/Web/API/Element/innerHTML), [Element.outerHTML](/en-US/docs/Web/API/Element/outerHTML), or [ShadowRoot.innerHTML](/en-US/docs/Web/API/ShadowRoot/innerHTML), for injecting untrusted HTML content. For example, in most case you can use [Element.setHTML()](/en-US/docs/Web/API/Element/setHTML) with the default sanitizer as a drop-in replacement for [Element.innerHTML](/en-US/docs/Web/API/Element/innerHTML). The same methods can also be used for injecting trusted HTML strings that do not need to contain any XSS-unsafe elements.
...
### [Sanitizer configuration](#sanitizer_configuration)
...
[SanitizerConfig](/en-US/docs/Web/API/SanitizerConfig) is a dictionary object that defines arrays for the allowed/disallowed elements and attributes and boolean properties that indicate whether comments and data attributes will be allowed or omitted, and so on.

Only a subset of possible configuration options may be specified in a particular configuration in order to reduce redundancy and ambiguity. The allowed subset is summarized in the [Allow and remove configurations](#allow_and_remove_configurations) section below, and described in detail in [Valid configuration](/en-US/docs/Web/API/SanitizerConfig#valid_configuration).

- 

[Sanitizer](/en-US/docs/Web/API/Sanitizer) is essentially a wrapper around a [SanitizerConfig](/en-US/docs/Web/API/SanitizerConfig) that provides methods to ergonomically modify the configuration and ensure that it remains valid.

For example, you can use a method to add an allowed element, and it will also remove the element from the `replaceWithChildrenElements` array (if present). The interface also provides methods to return a copy of the underlying [SanitizerConfig](/en-US/docs/Web/API/SanitizerConfig) and also to update the sanitizer so that it is XSS-safe. It may provide normalizations of the sanitizer configuration used to construct it, making it easier to understand and reuse.
...
Element arraysAttribute arraysValid?`elements`-✔️`elements``attributes`✔️`elements``removeAttributes`✔️`removeElements`-✔️`removeElements``attributes`✔️`removeElements``removeAttributes`✔️-`attributes`✔️-`removeAttributes`✔️`elements` + `removeElements`(anything)❌(anything)`attributes` + `removeAttributes`❌--✔️

An allow configuration can optionally specify whether per-element attributes should be allowed and/or removed in its `elements` array. The allowed configuration for these local attributes depends on whether or not global `attributes` or `removedAttributes` is defined. The [valid configuration](/en-US/docs/Web/API/SanitizerConfig#valid_configuration) section outlines the restrictions.
...
Allow configurations are created by defining a `Sanitizer` that wraps a [SanitizerConfig](/en-US/docs/Web/API/SanitizerConfig) that includes the [elements](/en-US/docs/Web/API/SanitizerConfig#elements) and/or [attributes](/en-US/docs/Web/API/SanitizerConfig#attributes) arrays (and not the `removeElements` or `removeAttributes` arrays).

For example, the following configuration is created by passing a [SanitizerConfig](/en-US/docs/Web/API/SanitizerConfig) that allows [<p>](/en-US/docs/Web/HTML/Reference/Elements/p) and [<div>](/en-US/docs/Web/HTML/Reference/Elements/div) elements, and `cite` and `onclick` attributes on any allowed element. It will also replace [<b>](/en-US/docs/Web/HTML/Reference/Elements/b) elements with their child nodes.
...
The same configuration can also be created using [Sanitizer](/en-US/docs/Web/API/Sanitizer) methods. Note that in the following code the `Sanitizer()` constructor takes an empty object, which results in a `Sanitizer` where the underlying configuration includes both `elements` and `attributes` arrays — in other words, an "allow configuration".
...
Remove configurations are created using a [SanitizerConfig](/en-US/docs/Web/API/SanitizerConfig) that includes the [removeElements](/en-US/docs/Web/API/SanitizerConfig#removeelements) and/or [removeAttributes](/en-US/docs/Web/API/SanitizerConfig#removeattributes) arrays (and not the `elements` or `attributes` arrays).
...
The configuration can also be created using [Sanitizer](/en-US/docs/Web/API/Sanitizer) methods. To make this a "remove configuration" we have to declare the `removeElements` or `removeAttributes` array when constructing the object (if only one array is specified the other will be defined as part of normalization).
...
### [Third party sanitization libraries](#third_party_sanitization_libraries)
...
[Sanitizer](/en-US/docs/Web/API/Sanitizer)Experimental
...
[SanitizerConfig](/en-US/docs/Web/API/SanitizerConfig)

A dictionary that defines a sanitizer configuration. This can be used in the same places as [Sanitizer](/en-US/docs/Web/API/Sanitizer) but is likely to be less efficient to use and reuse.

## [Extensions to other interfaces](#extensions_to_other_interfaces)

### [XSS-safe methods](#xss-safe_methods)

[Element.setHTML()](/en-US/docs/Web/API/Element/setHTML)
...
[ShadowRoot.setHTML()](/en-US/docs/Web/API/ShadowRoot/setHTML)
...
[Document.parseHTML()](/en-US/docs/Web/API/Document/parseHTML_static)

Parse a string of HTML into a subtree of nodes. Then drop any elements and attributes that are not allowed by the sanitizer configuration, and any that are considered XSS-unsafe (even if allowed by the configuration). The subtree is then set as the root of the [Document](/en-US/docs/Web/API/Document).

### [XSS-unsafe methods](#xss-unsafe_methods)
...
Parse a string of HTML into a subtree of nodes. Then drop any elements and attributes that are not allowed by the sanitizer: if no sanitizer is specified allow all elements. The subtree is then set as the root of the [Document](/en-US/docs/Web/API/Document).
...
### [Using Element.setHTML() with the default sanitizer](#using_element.sethtml_with_the_default_sanitizer)

In most cases calling `Element.setHTML()` without passing a sanitizer can be used as a drop-in replacement for [Element.innerHTML](/en-US/docs/Web/API/Element/innerHTML). The code below demonstrates how the method is used to sanitize the HTML input before it is injected into an element with id of `target`.
...
The [<script>](/en-US/docs/Web/HTML/Reference/Elements/script) element is not allowed by the default sanitizer, or by the `setHTML()` method, so the `alert()` is removed.
...
### [Using an allow sanitizer configuration](#using_an_allow_sanitizer_configuration)

This code shows how you might use `Element.setHTMLUnsafe()` with an allow sanitizer that allows only [<p>](/en-US/docs/Web/HTML/Reference/Elements/p), [<b>](/en-US/docs/Web/HTML/Reference/Elements/b), and [<div>](/en-US/docs/Web/HTML/Reference/Elements/div) elements. All other elements in the input string would be removed.
...
Was this page helpful to you?YesNo[Learn how to contribute](/en-US/docs/MDN/Community/Getting_started)
 ...
 This page was last modified on ⁨Dec 2, 2025⁩ by [MDN contributors](/en-US/docs/Web/API/HTML_Sanitizer_API/contributors.txt). 

[View this page on GitHub](https://github.com/mdn/content/blob/main/files/en-us/web/api/html_sanitizer_api/index.md?plain=1) • [Report a problem with this content](https://github.com/mdn/content/issues/new?template=page-report.yml&mdn-url=https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FHTML_Sanitizer_API&metadata=%3C%21--+Do+not+make+changes+below+this+line+--%3E%0A%3Cdetails%3E%0A%3Csummary%3EPage+report+details%3C%2Fsummary%3E%0A%0A*+Folder%3A+%60en-us%2Fweb%2Fapi%2Fhtml_sanitizer_api%60%0A*+MDN+URL%3A+https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FHTML_Sanitizer_API%0A*+GitHub+URL%3A+https%3A%2F%2Fgithub.com%2Fmdn%2Fcontent%2Fblob%2Fmain%2Ffiles%2Fen-us%2Fweb%2Fapi%2Fhtml_sanitizer_api%2Findex.md%0A*+Last+commit%3A+https%3A%2F%2Fgithub.com%2Fmdn%2Fcontent%2Fcommit%2F8b449a5846c1de417894acfe9b4471447181b57f%0A*+Document+last+modified%3A+2025-12-02T20%3A27%3A13.000Z%0A%0A%3C%2Fdetails%3E)
