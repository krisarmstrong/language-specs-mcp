...
The main API methods are [scheduler.postTask()](/en-US/docs/Web/API/Scheduler/postTask) and [scheduler.yield()](/en-US/docs/Web/API/Scheduler/yield). `scheduler.postTask()` takes a callback function (the task) and returns a promise that resolves with the return value of the function or rejects with an error. `scheduler.yield()` turns any [async](/en-US/docs/Web/JavaScript/Reference/Statements/async_function) function into a task by yielding the main thread to the browser for other work, with execution continuing when the returned promise is resolved.
...
### [scheduler.postTask()](#scheduler.posttask)
...
- `signal` This allows you to specify a signal, which may be either a [TaskSignal](/en-US/docs/Web/API/TaskSignal) or [AbortSignal](/en-US/docs/Web/API/AbortSignal) The signal is associated with a controller, which can be used to abort the task. A [TaskSignal](/en-US/docs/Web/API/TaskSignal) can also be used to set and change the task priority if the [task is mutable](#mutable_and_immutable_task_priority).
...
### [Mutable and immutable task priority](#mutable_and_immutable_task_priority)
...
Task priorities can be set as static (immutable) or dynamic (modifiable) depending on the arguments passed to [Scheduler.postTask()](/en-US/docs/Web/API/Scheduler/postTask).
...
The priority is modifiable only if a [TaskSignal](/en-US/docs/Web/API/TaskSignal) is passed to the `options.signal` argument and`options.priority` is not set. In this case the task will take its initial priority from the `signal` priority, and the priority can subsequently be changed by calling [TaskController.setPriority()](/en-US/docs/Web/API/TaskController/setPriority) on the controller associated with the signal.

If the priority is not set with `options.priority` or by passing a [TaskSignal](/en-US/docs/Web/API/TaskSignal) to `options.signal` then it defaults to `user-visible` (and is by definition immutable).

Note that a task that needs to be aborted must set `options.signal` to either [TaskSignal](/en-US/docs/Web/API/TaskSignal) or [AbortSignal](/en-US/docs/Web/API/AbortSignal). However for a task with an immutable priority, [AbortSignal](/en-US/docs/Web/API/AbortSignal) more clearly indicates that the task priority cannot be changed using the signal.
...
To improve this further, we can use [Scheduler.yield](/en-US/docs/Web/API/Scheduler/yield) when available to allow this code to continue executing ahead of other less critical tasks in the queue:
...
[Scheduler](/en-US/docs/Web/API/Scheduler)

Contains the [postTask()](/en-US/docs/Web/API/Scheduler/postTask) and [yield()](/en-US/docs/Web/API/Scheduler/yield) methods for adding prioritized tasks to be scheduled. An instance of this interface is available on the [Window](/en-US/docs/Web/API/Window) or [WorkerGlobalScope](/en-US/docs/Web/API/WorkerGlobalScope) global objects (`globalThis.scheduler`).

[TaskController](/en-US/docs/Web/API/TaskController)

Supports both aborting a task and changing its priority.

[TaskSignal](/en-US/docs/Web/API/TaskSignal)

A signal object that allows you to abort a task and change its priority, if required, using a [TaskController](/en-US/docs/Web/API/TaskController) object.

[TaskPriorityChangeEvent](/en-US/docs/Web/API/TaskPriorityChangeEvent)

The interface for the [prioritychange](/en-US/docs/Web/API/TaskSignal/prioritychange_event) event, which is sent when the priority for a task is changed.

Note: If the [task priority](#task_priorities) never needs to be changed, you can use an [AbortController](/en-US/docs/Web/API/AbortController) and its associated [AbortSignal](/en-US/docs/Web/API/AbortSignal) instead of [TaskController](/en-US/docs/Web/API/TaskController) and [TaskSignal](/en-US/docs/Web/API/TaskSignal).

### [Extensions to other interfaces](#extensions_to_other_interfaces)

[Window.scheduler](/en-US/docs/Web/API/Window/scheduler) and [WorkerGlobalScope.scheduler](/en-US/docs/Web/API/WorkerGlobalScope/scheduler)
...
### [Feature checking](#feature_checking)
...
Tasks are posted using [Scheduler.postTask()](/en-US/docs/Web/API/Scheduler/postTask), specifying a callback function (task) in the first argument, and an optional second argument that can be used to specify a task priority, signal, and/or delay. The method returns a [Promise](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) that resolves with the return value of the callback function, or rejects with either an abort error or an error thrown in the function.
...
Because it returns a promise, [Scheduler.postTask()](/en-US/docs/Web/API/Scheduler/postTask) can be [chained with other promises](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise#chained_promises). Below we show how to wait on the promise to resolve using [then](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/then). This uses the default priority (`user-visible`).
...
The method can also be used with [await](/en-US/docs/Web/JavaScript/Reference/Operators/await) inside an [async function](/en-US/docs/Web/JavaScript/Reference/Statements/async_function). The code below shows how you might use this approach to wait on a `user-blocking` task.
...
### [Permanent priorities](#permanent_priorities)

[Task priorities](#task_priorities) may be set using `priority` parameter in the optional second argument. Priorities that are set in this way are [immutable](#mutable_and_immutable_task_priority) (cannot be changed).
...
### [Changing task priorities](#changing_task_priorities)

[Task priorities](#task_priorities) can also take their initial value from a [TaskSignal](/en-US/docs/Web/API/TaskSignal) passed to `postTask()` in the optional second argument. If set in this way, the priority of the task [can then be changed](#mutable_and_immutable_task_priority) using the controller associated with the signal.

Note: Setting and changing task priorities using a signal only works when the `options.priority` argument to `postTask()` is not set, and when the `options.signal` is a [TaskSignal](/en-US/docs/Web/API/TaskSignal) (and not an [AbortSignal](/en-US/docs/Web/API/AbortSignal)).

The code below first shows how to create a [TaskController](/en-US/docs/Web/API/TaskController), setting the initial priority of its signal to `user-blocking` in the [TaskController()](/en-US/docs/Web/API/TaskController/TaskController) constructor.

The code then uses `addEventListener()` to add an event listener to the controller's signal (we could alternatively use the `TaskSignal.onprioritychange` property to add an event handler). The event handler uses [previousPriority](/en-US/docs/Web/API/TaskPriorityChangeEvent/previousPriority) on the event to get the original priority and [TaskSignal.priority](/en-US/docs/Web/API/TaskSignal/priority) on the event target to get the new/current priority.

The task is then posted, passing in the signal, and then we immediately change the priority to `background` by calling [TaskController.setPriority()](/en-US/docs/Web/API/TaskController/setPriority) on the controller.
...
### [Aborting tasks](#aborting_tasks)

Tasks can be aborted using either [TaskController](/en-US/docs/Web/API/TaskController) and [AbortController](/en-US/docs/Web/API/AbortController), in exactly the same way. The only difference is that you must use [TaskController](/en-US/docs/Web/API/TaskController) if you also want to set the task priority.
...
The code below creates a controller and passes its signal to the task. The task is then immediately aborted. This causes the promise to be rejected with an `AbortError`, which is caught in the `catch` block and logged. Note that we could also have listened for the [abort](/en-US/docs/Web/API/AbortSignal/abort_event) event fired on the [TaskSignal](/en-US/docs/Web/API/TaskSignal) or [AbortSignal](/en-US/docs/Web/API/AbortSignal) and logged the abort there.
...
Was this page helpful to you?YesNo[Learn how to contribute](/en-US/docs/MDN/Community/Getting_started)
 ...
 This page was last modified on ⁨Jun 23, 2025⁩ by [MDN contributors](/en-US/docs/Web/API/Prioritized_Task_Scheduling_API/contributors.txt). 

[View this page on GitHub](https://github.com/mdn/content/blob/main/files/en-us/web/api/prioritized_task_scheduling_api/index.md?plain=1) • [Report a problem with this content](https://github.com/mdn/content/issues/new?template=page-report.yml&mdn-url=https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FPrioritized_Task_Scheduling_API&metadata=%3C%21--+Do+not+make+changes+below+this+line+--%3E%0A%3Cdetails%3E%0A%3Csummary%3EPage+report+details%3C%2Fsummary%3E%0A%0A*+Folder%3A+%60en-us%2Fweb%2Fapi%2Fprioritized_task_scheduling_api%60%0A*+MDN+URL%3A+https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FPrioritized_Task_Scheduling_API%0A*+GitHub+URL%3A+https%3A%2F%2Fgithub.com%2Fmdn%2Fcontent%2Fblob%2Fmain%2Ffiles%2Fen-us%2Fweb%2Fapi%2Fprioritized_task_scheduling_api%2Findex.md%0A*+Last+commit%3A+https%3A%2F%2Fgithub.com%2Fmdn%2Fcontent%2Fcommit%2F3e543cdfe8dddfb4774a64bf3decdcbab42a4111%0A*+Document+last+modified%3A+2025-06-23T16%3A41%3A39.000Z%0A%0A%3C%2Fdetails%3E)
