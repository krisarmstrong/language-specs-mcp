...
- [Method/properties overview](#methodproperties_overview)
- [Handling promise and non-promise versions of requestPointerLock()](#handling_promise_and_non-promise_versions_of_requestpointerlock)
- [pointerlockchange event](#pointerlockchange_event)
- [pointerlockerror event](#pointerlockerror_event)
- [Extensions to mouse events](#extensions_to_mouse_events)
- [Simple example walkthrough](#simple_example_walkthrough)
- [IFrame limitations](#iframe_limitations)
...
The Pointer lock API, similar to the [Fullscreen API](/en-US/docs/Web/API/Fullscreen_API), extends DOM elements by adding a new method, [requestPointerLock()](/en-US/docs/Web/API/Element/requestPointerLock). The following example requests pointer lock on a [<canvas>](/en-US/docs/Web/HTML/Reference/Elements/canvas) element:
...
### [pointerLockElement and exitPointerLock()](#pointerlockelement_and_exitpointerlock)

The Pointer lock API also extends the [Document](/en-US/docs/Web/API/Document) interface, adding a new property and a new method:

- [pointerLockElement](/en-US/docs/Web/API/Document/pointerLockElement) is used for accessing the currently locked element (if any).
- [exitPointerLock()](/en-US/docs/Web/API/Document/exitPointerLock) is used to exit the pointer lock.

The [pointerLockElement](/en-US/docs/Web/API/Document/pointerLockElement) property is useful for determining if any element is currently pointer locked (e.g., for doing a boolean check) and also for obtaining a reference to the locked element, if any.
...
The [Document.exitPointerLock()](/en-US/docs/Web/API/Document/exitPointerLock) method is used to exit pointer lock, and like [requestPointerLock](/en-US/docs/Web/API/Element/requestPointerLock), works asynchronously using the [pointerlockchange](/en-US/docs/Web/API/Document/pointerlockchange_event) and [pointerlockerror](/en-US/docs/Web/API/Document/pointerlockerror_event) events, which you'll see more about below.
...
When the Pointer lock state changes—for example, when calling [requestPointerLock()](/en-US/docs/Web/API/Element/requestPointerLock) or [exitPointerLock()](/en-US/docs/Web/API/Document/exitPointerLock), the user pressing the ESC key, etc.—the [pointerlockchange](/en-US/docs/Web/API/Document/pointerlockchange_event) event is dispatched to the `document`. This is a simple event containing no extra data.
...
When there is an error caused by calling [requestPointerLock()](/en-US/docs/Web/API/Element/requestPointerLock) or [exitPointerLock()](/en-US/docs/Web/API/Document/exitPointerLock), the [pointerlockerror](/en-US/docs/Web/API/Document/pointerlockerror_event) event is dispatched to the `document`. This is a simple event containing no extra data.
...
The Pointer lock API extends the normal [MouseEvent](/en-US/docs/Web/API/MouseEvent) interface with movement attributes. Two new attributes to mouse events—[movementX](/en-US/docs/Web/API/MouseEvent/movementX) and [movementY](/en-US/docs/Web/API/MouseEvent/movementY)—provide the change in mouse positions. The values of the parameters are the same as the difference between the values of [MouseEvent](/en-US/docs/Web/API/MouseEvent) properties, [screenX](/en-US/docs/Web/API/MouseEvent/screenX) and [screenY](/en-US/docs/Web/API/MouseEvent/screenY), which are stored in two subsequent [mousemove](/en-US/docs/Web/API/Element/mousemove_event) events, `eNow` and `ePrevious`. In other words, the Pointer lock parameter `movementX = eNow.screenX - ePrevious.screenX`.

### [Locked state](#locked_state)

When Pointer lock is enabled, the standard [MouseEvent](/en-US/docs/Web/API/MouseEvent) properties [clientX](/en-US/docs/Web/API/MouseEvent/clientX), [clientY](/en-US/docs/Web/API/MouseEvent/clientY), [screenX](/en-US/docs/Web/API/MouseEvent/screenX), and [screenY](/en-US/docs/Web/API/MouseEvent/screenY) are held constant, as if the mouse is not moving. The [movementX](/en-US/docs/Web/API/MouseEvent/movementX) and [movementY](/en-US/docs/Web/API/MouseEvent/movementY) properties continue to provide the mouse's change in position. There is no limit to [movementX](/en-US/docs/Web/API/MouseEvent/movementX) and [movementY](/en-US/docs/Web/API/MouseEvent/movementY) values if the mouse is continuously moving in a single direction. The concept of the mouse cursor does not exist and the cursor cannot move off the window or be clamped by a screen edge.

### [Unlocked state](#unlocked_state)

The parameters [movementX](/en-US/docs/Web/API/MouseEvent/movementX) and [movementY](/en-US/docs/Web/API/MouseEvent/movementY) are valid regardless of the mouse lock state, and are available even when unlocked for convenience.

When the mouse is unlocked, the system cursor can exit and re-enter the browser window. If that happens, [movementX](/en-US/docs/Web/API/MouseEvent/movementX) and [movementY](/en-US/docs/Web/API/MouseEvent/movementY) could be set to zero.

## [Simple example walkthrough](#simple_example_walkthrough)

We've written a [pointer lock demo](https://mdn.github.io/dom-examples/pointer-lock/) ([see source code](https://github.com/mdn/dom-examples/tree/main/pointer-lock)) to show you how to use it to set up a simple control system. This demo uses JavaScript to draw a ball on top of a [<canvas>](/en-US/docs/Web/HTML/Reference/Elements/canvas) element. When you click the canvas, pointer lock is then used to remove the mouse pointer and allow you to move the ball directly using the mouse. Let's see how this works.
...
Pointer lock can only lock one [<iframe>](/en-US/docs/Web/HTML/Reference/Elements/iframe) at a time. If you lock one `<iframe>`, you can't lock another one and transfer the target to it; pointer lock will error out. To avoid this limitation, first unlock the locked `<iframe>`, and then lock the other.
...
Specification[Pointer Lock 2.0](https://w3c.github.io/pointerlock/)

## [Browser compatibility](#browser_compatibility)

### [api.Document.exitPointerLock](#api.Document.exitPointerLock)

### [api.Element.requestPointerLock](#api.Element.requestPointerLock)
