...
Touch events are similar to mouse events except they support simultaneous touches and at different locations on the touch surface. The [TouchEvent](/en-US/docs/Web/API/TouchEvent) interface encapsulates all of the touchpoints that are currently active. The [Touch](/en-US/docs/Web/API/Touch) interface, which represents a single touchpoint, includes information such as the position of the touch point relative to the browser viewport.
...
### [Drawing application](#drawing_application)

This example tracks multiple touchpoints at a time, allowing the user to draw in a [<canvas>](/en-US/docs/Web/HTML/Reference/Elements/canvas) with more than one finger at a time. It will only work on a browser that supports touch events.

#### Create a canvas

The HTML consists of a single [<canvas>](/en-US/docs/Web/HTML/Reference/Elements/canvas) element. Curves will be drawn in response to the user's touch gestures. A button is also included to clear the canvas.
...
When a [touchstart](/en-US/docs/Web/API/Element/touchstart_event) event occurs, indicating that a new touch on the surface has occurred, the `handleStart()` function below is called.
...
This calls [event.preventDefault()](/en-US/docs/Web/API/Event/preventDefault) to keep the browser from continuing to process the touch event (this also prevents a mouse event from also being delivered). Then we get the context and pull the list of changed touch points out of the event's [TouchEvent.changedTouches](/en-US/docs/Web/API/TouchEvent/changedTouches) property.

After that, we iterate over all the [Touch](/en-US/docs/Web/API/Touch) objects in the list, pushing them onto an array of active touchpoints and drawing the start point for the draw as a small circle; we're using a 4-pixel wide line, so a 4-pixel radius circle will show up neatly.
...
The result from this function is a string that can be used when calling [<canvas>](/en-US/docs/Web/HTML/Reference/Elements/canvas) functions to set drawing colors. For example, for a [Touch.identifier](/en-US/docs/Web/API/Touch/identifier) value of 10, the resulting string is "#aa3311".

#### Handling the end of a touch

When the user lifts a finger off the surface, a [touchend](/en-US/docs/Web/API/Element/touchend_event) event is sent. We handle this by calling the `handleEnd()` function below. Its job is to draw the last line segment for each touch that ended and remove the touchpoint from the ongoing touch map.
...
If the user's finger wanders into browser UI, or the touch otherwise needs to be canceled, the [touchcancel](/en-US/docs/Web/API/Element/touchcancel_event) event is sent, and we call the `handleCancel()` function below.
...
### [Calling preventDefault() only on a second touch](#calling_preventdefault_only_on_a_second_touch)
...
Specification[Touch Events](https://w3c.github.io/touch-events/)
...
### [api.TouchEvent](#api.TouchEvent)

### [api.TouchList](#api.TouchList)

## Help improve MDN

Was this page helpful to you?YesNo[Learn how to contribute](/en-US/docs/MDN/Community/Getting_started)

 This page was last modified on ⁨Oct 21, 2025⁩ by [MDN contributors](/en-US/docs/Web/API/Touch_events/contributors.txt). 

[View this page on GitHub](https://github.com/mdn/content/blob/main/files/en-us/web/api/touch_events/index.md?plain=1) • [Report a problem with this content](https://github.com/mdn/content/issues/new?template=page-report.yml&mdn-url=https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FTouch_events&metadata=%3C%21--+Do+not+make+changes+below+this+line+--%3E%0A%3Cdetails%3E%0A%3Csummary%3EPage+report+details%3C%2Fsummary%3E%0A%0A*+Folder%3A+%60en-us%2Fweb%2Fapi%2Ftouch_events%60%0A*+MDN+URL%3A+https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FTouch_events%0A*+GitHub+URL%3A+https%3A%2F%2Fgithub.com%2Fmdn%2Fcontent%2Fblob%2Fmain%2Ffiles%2Fen-us%2Fweb%2Fapi%2Ftouch_events%2Findex.md%0A*+Last+commit%3A+https%3A%2F%2Fgithub.com%2Fmdn%2Fcontent%2Fcommit%2F36761819df2ebdd4e3dcc9ae6007029dec71fac0%0A*+Document+last+modified%3A+2025-10-21T22%3A19%3A42.000Z%0A%0A%3C%2Fdetails%3E)
